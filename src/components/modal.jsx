...
export function Modal({envi, setRoomID, setName, setOccupants}) {
  ...
  async function createRoom(name) {
    let data = envi === 'rtcbun'
    ? await fetch(`https://rtcbun.site:5000/?name=${name}`)
    : await fetch(`http://localhost:5000/?name=${name}`);
  ...
  async function joinRoom(name, roomToJoin) {
    let data = envi === 'rtcbun'
    ? await fetch(`https://rtcbun.site:5000/?name=${name}&roomReq=${roomToJoin}`)
    : await fetch(`http://localhost:5000/?name=${name}&roomReq=${roomToJoin}`);
    data = await data.json();
    if (data.status !== 200) {
      alert(data.message)
    }
